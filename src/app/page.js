...
const SocialLink = ({ icon, label, href, isDarkMode }) => (
  <div className="flex items-center mb-4 w-full">
    <FontAwesomeIcon icon={icon} className="mr-2" aria-hidden="true" />
    <span className="text-medium font-semibold">{label}</span>
    <span 
      className={`flex-grow border-t border-dotted ${isDarkMode ? 'border-gray-300' : 'border-gray-400'} mx-4`}
      aria-hidden="true"
    />
    <a 
      href={href}
      className={`${isDarkMode ? 'text-gray-300' : 'text-gray-400'} hover:text-orange-500`}
